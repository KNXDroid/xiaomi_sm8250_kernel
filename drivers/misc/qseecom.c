					 ...
					 bool *perf_enabled)
{
	...
	*perf_enabled = false;
	...
	/*
	 * On targets where crypto clock is handled by HLOS, if clk_access_cnt
	 * is zero and perf_enabled is false, vote clocks for this command.
	 */
	if (!qseecom.no_clock_support &&
		!qseecom.qsee.clk_access_cnt && !data->perf_enabled) {
		pr_debug("ce clock is not enabled!\n");
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case QSEECOM_IOCTL_SEND_CMD_REQ: {
		...
		mutex_lock(&app_access_lock);
		ret = __qseecom_prepare_client_send(data, &perf_enabled);
		if (ret) {
			mutex_unlock(&app_access_lock);
			break;
		}
		ret = qseecom_send_cmd(data, argp);
		__qseecom_finish_client_send(data, perf_enabled);
		mutex_unlock(&app_access_lock);
		if (ret)
